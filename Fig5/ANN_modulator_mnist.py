...
def MG_readout(gain, act, mod, tau=1, norm=True, device='cpu'):
    with torch.no_grad():
        ...
        '''
        # standardize act first
        std = torch.sqrt(torch.var(act,axis=0)).squeeze(-1)
        actstand[:,std==0] = 0
        std[std==0] = 1
        actstand = actstand/std'''
        a = (torch.mm((actstand).T, (mod-torch.mean(mod))) / mod.shape[0]).T
        #a = (torch.mm((actstand).T,(mod-torch.mean(mod)))/mod.shape[0]).T
        '''std = (torch.sqrt(torch.var(act,axis=0))).squeeze(-1) # *torch.sqrt(torch.var(mod))
        a[0,std==0] = 0
        std[std==0] = 1
        a = a/std
        if np.sum(torch.abs(a).detach().numpy()>1)>0: print('error in correlation measure')'''
...
def vis_d_w(data, label, s1, s2, upsamp, tasks, tt, W, dtype, device, net,
            fig=None, ax=None, var='w', d_layer = 0):
    # d' bins
    bins = np.linspace(-6-d_layer*2, 6+d_layer*2, 20)

    # only s1
    x1 = torch.tensor(data[label==s1], dtype=dtype, device=device)
    act_s1=(net.layer_enc(x1).detach().numpy())
    # only s2
    x2 = torch.tensor(data[label==s2], dtype=dtype, device=device)
    act_s2=(net.layer_enc(x2).detach().numpy())
    if d_layer == 1:
        act_s1 = net.hidden1(net.relu(net.layer_enc(x1))).detach().numpy()
        act_s2 = net.hidden1(net.relu(net.layer_enc(x2))).detach().numpy()
    # compute d
    d = (np.mean(act_s1,axis=(0))-np.mean(act_s2,axis=(0)))/\
        np.sqrt(.5*(np.var(act_s1,axis=(0))+np.var(act_s2,axis=(0))))
    d[np.abs(np.mean(act_s1,axis=(0))-np.mean(act_s2,axis=(0)))<1e-3] = 0
    # comparison d' and w
    w = np.mean(W,axis=0) 
    if fig is None:
        fig, ax = plt.subplots(1+(d_layer==0),3,figsize=(9,3+(d_layer==0)*4))
    if d_layer == 1:
        ax[0].plot(w, (d), '.')
        ax[1].plot(w, np.abs(d), '.')
        ax[2].hist(d, bins=bins, histtype='step')
        ax[0].set_xlabel(var)
        ax[1].set_xlabel(var)
        ax[0].set_ylabel('d')
        ax[1].set_ylabel('|d|')
        ax[2].set_title('distribution of |d|')
    print('corr ' + var + ' |d| ', np.corrcoef(w, np.abs(d))[0, 1])
    print('corr '+var+' d ', np.corrcoef(w, (d))[0,1])
    if d_layer==0:
        ax[0][0].plot(w, (d), '.')
        ax[0][1].plot(w, np.abs(d), '.')
        ax[0][2].hist(d, bins=bins, histtype='step')
        ax[0][0].set_xlabel(var)
        ax[0][1].set_xlabel(var)
        ax[0][0].set_ylabel('d')
        ax[0][1].set_ylabel('|d|')
        ax[0][2].set_title('distribution of |d|')
        ax[1][0].plot(w[(upsamp*tasks[tt]):(upsamp*(tasks[tt]+1))], d[(upsamp*tasks[tt]):(upsamp*(tasks[tt]+1))], '.')
        ax[1][1].plot(w[(upsamp*tasks[tt]):(upsamp*(tasks[tt]+1))], np.abs(d)[(upsamp*tasks[tt]):(upsamp*(tasks[tt]+1))], '.')
        ax[1][2].hist(d[(upsamp*tasks[tt]):(upsamp*(tasks[tt]+1))], bins=bins, histtype='step')
        ax[1][0].set_xlabel(var)
        ax[1][1].set_xlabel(var)
        ax[1][0].set_ylabel('d')
        ax[1][1].set_ylabel('|d|')
        ax[1][2].set_title('distribution of |d|')
        print('corr ' + var + ' |d| just for spatially informative neurons ',
              np.corrcoef(w[(upsamp * tasks[tt]):(upsamp * (tasks[tt] + 1))],
                          np.abs(d)[(upsamp * tasks[tt]):(upsamp * (tasks[tt] + 1))])[0, 1])
        print('corr ' + var + ' d just for spatially informative neurons ',
              np.corrcoef(w[(upsamp * tasks[tt]):(upsamp * (tasks[tt] + 1))], \
                          (d)[(upsamp * tasks[tt]):(upsamp * (tasks[tt] + 1))])[0, 1])
    fig.tight_layout()
...
def produce_dataset(Gpar_data, nsamplespre, nsamplestun, testtoo,
    ...
    if Gpar_data['data_type']=='gabor':
        ...
        if task is not None:
            ...
            # replicate to increase the size of the dataset
            nn = 1
            while datatask.shape[0] < nsamplestun:
                datatask, labeltask = np.repeat(datatask, nn, axis=0), np.repeat(labeltask, nn, axis=0)
                nn +=1
            if testtoo:
                nn = 1
